...
function buildExampleText(item) {
  ...
  if (examples && examples.length > 0) {
    return examples
      .slice(0, 2)
      .map((example) => normalizeExampleWord(item, example))
      .join(' / ');
  }
...
function buildKoreanInfoText(item) {
  ...
  if (meanings.length === readings.length && meanings.length > 0) {
    return meanings
      .map((meaning, index) => `${meaning} ${readings[index]}`)
      .join(' / ');
  }
...
const toggleModeButtons = Array.from(
  document.querySelectorAll('[data-toggle-mode]'),
);
const holdModeButtons = Array.from(
  document.querySelectorAll('[data-hold-mode]'),
);
...
const selectionActionCountEl = selectionActionPopover.querySelector(
  '[data-selection-count]',
);
const selectionActionMemorizeBtn = selectionActionPopover.querySelector(
  "[data-selection-action='memorize']",
);
const selectionActionUnmemorizeBtn = selectionActionPopover.querySelector(
  "[data-selection-action='unmemorize']",
);
const selectionActionCloseBtn = selectionActionPopover.querySelector(
...
function loadMemorizedKanji() {
  try {
    ...
    console.warn(
      'Could not load memorized kanji state from localStorage.',
      error,
    );
    return new Set();
  }
...
function saveMemorizedKanji() {
  try {
    localStorage.setItem(
      MEMORIZED_KANJI_STORAGE_KEY,
      JSON.stringify([...memorizedKanji]),
    );
  } catch (error) {
    console.warn(
      'Could not save memorized kanji state to localStorage.',
      error,
    );
  }
}
...
function positionSelectionActionPopover() {
  ...
  const selectedRects = [...tempSelectedCells].map((cell) =>
...
function updateSelectionMarquee() {
  if (
    !isRectSelectionMode ||
    !dragSelectionState.isDragging ||
    !dragSelectionState.hasDragged
  ) {
    hideSelectionMarquee();
    return;
  }
  ...
  const width = Math.abs(
    dragSelectionState.currentX - dragSelectionState.startX,
  );
  const height = Math.abs(
    dragSelectionState.currentY - dragSelectionState.startY,
  );
...
function updateSelectionShapeToggleUI() {
  ...
  selectionShapeToggleBtn.setAttribute(
    'aria-pressed',
    isRectSelectionMode ? 'true' : 'false',
  );
  selectionShapeToggleBtn.dataset.shapeMode = isRectSelectionMode
    ? 'rect'
    : 'path';
...
async function initializeKanjiList() {
  try {
    ...
    if (joyoKanji.length < TARGET_KANJI_COUNT) {
      throw new Error(
        `sorted joyo kanji payload too short: ${joyoKanji.length}`,
      );
    }
    if (joyoKanji.length > TARGET_KANJI_COUNT) {
      console.warn(
